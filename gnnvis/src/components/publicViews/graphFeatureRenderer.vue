<!--
@description this view is for graph features which are usually defined by users
             thus, feature data are fetched asynchronously by click the button
-->
<template>
    <div
        :style="{
    ...
    >
        <div
            v-if="
                isEmptyDict(ds.graphUserDefinedFeatureRecord) && !view.isLoading
            "
        >
            <p>no graph feature added, click to fetch</p>
            <div>
                <el-button @click="view.loadingProcess">Fetch</el-button>
            </div>
        </div>
        <LoadingComp
            v-else-if="view.isLoading"
            text="fetching graph features..."
        />
        <ErrorComp v-else-if="view.loadingError" :error="view.loadingError" />
                    ...
                    "
                            ...
                            >
                                <text
                                    ...
                                    cursor="pointer"
                                    @mouseenter="
                                        db.highlightedWholeGraphIds = d.reduce(
                                            (acc, cur) => ({
                                                ...acc,
                                                [cur.id]: true,
                                            }),
                                ...
                                >
                                    ...
                                    {{ d.length > 0 ? d.length : "" }}
                                    <title>
                                        double click to select this bar
                                    </title>
                                </text>
                            </g>
                        </g>
...
import { isEmptyDict, nodeMapGraph2GraphMapNodes } from "@/utils/graphUtils";
...
const view = myStore.getViewByName(db, props.viewName) as DenseView & AsyncView;
...
const graphMapParentDbIndex = (gid: Type_GraphId) => {
    let has0 = false;
    let has1 = false;
    for (const nodeInfo of Object.values(srcGraphsDict.value[gid])) {
...
const graphsDictFromDb0 = computed(() => {
    const ret = {} as Record<
        Type_GraphId,
    ...
    for (const gid in srcGraphsDict.value) {
        const parentDbIndex = graphMapParentDbIndex(gid);
        if (parentDbIndex === 0 || parentDbIndex === 2) {
            ret[gid] = srcGraphsDict.value[gid];
        }
    }
    return ret;
});
const graphsDictFromDb1 = computed(() => {
    const ret = {} as Record<
        Type_GraphId,
        Record<Type_NodeId, { gid: string; parentDbIndex: number; hop: number }>
    >;
...
const originFeatureNames = computed(() =>
    Object.keys(ds.graphUserDefinedFeatureRecord)
);
const featureNames = computed(() =>
    isDbWiseComparativeDb(db)
        ? originFeatureNames.value.flatMap((name) => [
              name + "_db0",
              name + "_db1",
          ])
        : originFeatureNames.value
);

const featureGroupByNames = computed(() =>
    isDbWiseComparativeDb(db)
        ? originFeatureNames.value.flatMap((name) => {
              const featureRecord = ds.graphUserDefinedFeatureRecord[name];
              return [
                  Object.entries(featureRecord)
                      .filter((d) => graphsDictFromDb0.value[d[0]])
                      .map((d) => ({ id: d[0], feature: d[1] })),
                  Object.entries(featureRecord)
                      .filter((d) => graphsDictFromDb1.value[d[0]])
                      .map((d) => ({ id: d[0], feature: d[1] })),
              ];
          })
        : originFeatureNames.value.map((name) => {
              ...
              return Object.entries(featureRecord)
                  .filter((d) => srcGraphsDict.value[d[0]])
                  .map((d) => ({ id: d[0], feature: d[1] }));
          })
); // [
// [ {id: '0', feature:555}, {id: '123', feature: 888}], //feat0
// [ {id: '0', feature:33}, {id: '123', feature: 22}], //feat1
// ]
...
const colorScale = ds.graphColorScale || (() => "#888");
...
const titleHeight = computed(() => 0.08 * subHeight.value);
