...
def evaluation():
    ...
    for epoch in range(config.num_epoch, config.num_epoch + 1):
        ...
        load_checkpoint(config.ckptpath + "/bgcf_epoch{}.ckpt".format(epoch), net=bgcfnet_test)
        forward_net = ForwardBGCF(bgcfnet_test)
        forward_net.set_train(False)
