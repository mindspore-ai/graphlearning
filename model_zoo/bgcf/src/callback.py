...
def TestBGCF(forward_net, num_user, num_item, input_dim, test_graph_dataset):
    ...
    for _ in range(50):
        ...
        u_test_neighs = Tensor(u_test_neighs, mstype.int32)
        u_test_gnew_neighs = Tensor(u_test_gnew_neighs, mstype.int32)
        ...
        items = Tensor(np.arange(num_item).reshape(-1,) + num_user, mstype.int32)
        neg_items = Tensor(np.arange(num_item).reshape(-1, 1) + num_user, mstype.int32)
...
class BGCFLoss(nn.Cell):
    ...
    def construct(self, all_user_embed, all_user_rep, all_pos_item_embed,
        ...
        """Calculate loss"""
        all_user_embed = self.cast(all_user_embed, mstype.float32)
        ...
        all_pos_item_embed = self.cast(all_pos_item_embed, mstype.float32)
        ...
        neg_item_embed = self.cast(neg_item_embed, mstype.float32)
        ...
        # print(bpr_loss, reg_loss, user_user_distance, pos_item_item_distance)
