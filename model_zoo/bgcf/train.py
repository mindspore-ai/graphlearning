...
def modelarts_pre_process():
    ...
    if not os.path.exists(config.ckptpath):
...
def run_train():
    ...
    for epoch in range(1, config.num_epoch + 1):
        ...
        for data in itr:
            ...
            pos_item_id = Tensor(data["items"], mstype.int32)
            neg_item_id = Tensor(data["neg_item_id"], mstype.int32)
            pos_users = Tensor(data["pos_users"], mstype.int32)
            pos_items = Tensor(data["pos_items"], mstype.int32)

            u_group_nodes = Tensor(data["u_group_nodes"], mstype.int32)
            u_neighs = Tensor(data["u_neighs"], mstype.int32)
            u_gnew_neighs = Tensor(data["u_gnew_neighs"], mstype.int32)

            i_group_nodes = Tensor(data["i_group_nodes"], mstype.int32)
            i_neighs = Tensor(data["i_neighs"], mstype.int32)
            i_gnew_neighs = Tensor(data["i_gnew_neighs"], mstype.int32)

            neg_group_nodes = Tensor(data["neg_group_nodes"], mstype.int32)
            ...
            if iter_num == num_iter:
                print('Epoch', '%03d' % epoch, 'iter', '%02d' % iter_num,
                      'loss',
                      '{}, cost:{:.4f}'.format(train_loss, time.time() - epoch_start))
            iter_num += 1

        if epoch % config.eval_interval == 0:
            save_checkpoint(bgcfnet, config.ckptpath + "/bgcf_epoch{}.ckpt".format(epoch))
