...
def test_encoder():
    ...
    dst_idx = ms.Tensor([2, 6, 5, 4, 3, 2, 1], ms.int32)
...
def test_decoder():
    ...
    n_edges = 6
    src_idx = ms.Tensor([0, 1, 2, 3, 4, 5, 6], ms.int32)
    dst_idx = ms.Tensor([3, 6, 5, 4, 3, 2, 1], ms.int32)
    index = 0
    g = GraphField(ms.Tensor(src_idx, dtype=ms.int32), ms.Tensor(dst_idx, dtype=ms.int32),
                   int(n_nodes), int(n_edges))
...
def test_vgae_model():
    """
    Feature:Test whether the input and output of the vgae model module are correct.
    ...
    encoder = GCNEncoder(4,
                         ...
                         name='VGAE')
    decoder = InnerProductDecoder()
    ret, mean, std = VGAENet(encoder, decoder)(node_feat, in_deg, out_deg, index, *g.get_graph())
    ...
    assert np.array(mean).shape == np.array(expected_mean).shape

    assert np.array(std).shape == np.array(expected_std).shape
